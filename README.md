...
Individual steps can also be run separately — see `scripts/etl_pipeline.py` for the exact order and
commands, or run the corresponding scripts in `scripts/` directly.
...
```
`04_performance_analytics.ipynb` produces `fund_scorecard.csv` and `alpha_beta.csv` (needed by
the dashboard and by `scripts/recommender.py`) — run it before `05_advanced_analytics.ipynb`.

`05_advanced_analytics.ipynb` produces `var_cvar_report.csv`, `rolling_sharpe_chart.png`,
...
self-contained **HTML/Plotly** file; a `.pbix` or `.twbx` file is not included. It has all 4 required pages with
...
To regenerate `dashboard_data.json` after re-running the pipeline: `python scripts/build_dashboard_data.py`.
...
│   ├── processed/        ← cleaned CSVs (generated by scripts/etl_pipeline.py)
